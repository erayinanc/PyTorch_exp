...
# std libs
import argparse, sys, os, time, numpy as np, logging, matplotlib.pyplot as plt, h5py, scipy as sp
...
def pars_ini():
    global args
    parser = argparse.ArgumentParser(description='Train MNIST with DPM model to test possible image generation -- experimental')
...
class DPM(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(20, 40, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn3 = nn.BatchNorm2d(40)
        self.conv4 = nn.Conv2d(40, 80, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn4 = nn.BatchNorm2d(80)

        # decode
        self.conv5 = nn.Conv2d(80, 40, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn5 = nn.BatchNorm2d(40)
        self.conv6 = nn.Conv2d(40, 20, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn6 = nn.BatchNorm2d(20)
        self.conv7 = nn.Conv2d(20, 10, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn7 = nn.BatchNorm2d(10)
        self.conv8 = nn.Conv2d(10, 1, kernel_size=3, stride=1, padding=1, bias=False)
...
def train(model, device, train_loader, optimizer, epoch, loss_function, scheduler):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        res = diff_model(data, epoch)
        lt_2 += time.perf_counter() - lt_3

        with torch.set_grad_enabled(True):
            # forward pass
            output = model(res.data_dpm.to(device)).float()
...
class diff_model:
    def __init__(self,data,epoch=1,sigma=1.0,eps=1e-1):
        # start a timer
        ...
        sigma =  2.0 if epoch >  1 else sigma
        sigma =  3.0 if epoch >  2 else sigma
        sigma =  4.0 if epoch >  3 else sigma
        sigma =  5.0 if epoch >  4 else sigma
        self.sigma = sigma

        # apply filter with added noise for diffusion
        self.data_dpm = torch.clone(data)
        if sigma>1.01:
            ...
            for i in range(m):
                for j in range(n):
                    res = sp.ndimage.gaussian_filter(data[i,j,:,:], self.sigma, truncate=1.0)
                    self.data_dpm[i,j,:,:] = torch.from_numpy(res + noise_map*eps)
...
def noise_gen(xpix,ypix):
    ...
    noise_map = [[noise([i/xpix, j/ypix]) for i in range(xpix)] for j in range(ypix)]
...
def test(model, device, test_loader, loss_function):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            res = diff_model(data, epoch=1e9, sigma=10)
            output = model(res.data_dpm.to(device)).float()
...
def plot_scatter(inp_img, out_img):
    ...
    plt.savefig('recon_DPM.png', bbox_inches = 'tight', pad_inches = 0)
...
def plot_scatter_test(inp_img, org_img, out_img, epoch):
    ...
    fig.tight_layout(pad=1.0)
    plt.savefig('recon_DPM_train_'+str(epoch)+'.pdf',bbox_inches='tight',pad_inches=0)
